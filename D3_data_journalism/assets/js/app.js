...
function renderCircles(circlesGroup, newXScale, chosenXAxis) {
  ...
  function updateToolTip(chosenXAxis, circlesGroup) {
    ...
    else { //PROBABLY NEEDS ELSE IF BC 2 MORE AXIS: AGE(MEDIAN) & HOUSEHOLD INCOME(MEDIAN)
      label = "Age (Median)"; //three if / elsle if / else
    }
  
    var toolTip = d3.tip() //tooltip shows the data, place tooltip 
      .attr("class", "tooltip")
      .offset([80, -60])
...
d3.csv("assets/data/data.csv").then(function(data, err) {
  ...
  //change if not equal. BC when ITS NOT U LISTEN TO CHANGE & reload chart
          ...
          else { //else if chosemAxis ==== poverty =>three labels; else {bc defaults to healthcare#3}
            ageLabel
              .classed("active", false)
              .classed("inactive", true);
            povertyLabel
